...
class ChessTheoryApp {
  ...
  resetGameState() {
    // Cancel all pending API requests before resetting
    if (typeof ChessAPI !== 'undefined' && ChessAPI.cancelAllRequests) {
      ChessAPI.cancelAllRequests();
    }
  ...
  async queryExplorer() {
    ...
    try {
      ...
      // Don't stop the game on query failure
      const countEl = document.getElementById('gameCount');
      if (countEl) {
        countEl.textContent = 'Position data temporarily unavailable';
      }
    }
  }
  ...
  async aiMove() {
    ...
    try {
      ...
      // Check if we have valid moves data
      if (totalGames < minGames || !data.moves || data.moves.length === 0) {
        console.log(`📊 Position has ${totalGames} games - fetching game data to end battle...`);
      ...
      if (move) {
        ...
        this.ui.renderBoard();
        await this.queryExplorer();
      }
    } catch (error) {
      console.error('aiMove error:', error);
      // On critical error, end the game gracefully
      if (this.playerMoves > 0) {
        console.log('⚠️ Ending game due to API error');
        this.topGames = [];
        this.recentGames = [];
        await this.stopGameDueToThinTheory();
      }
